...
public class PsylinkThrowable : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        RaycastHit hit;
        psylinkInSight = Physics.Raycast(cam.position, cam.forward, out hit, 20f) && hit.transform.CompareTag("PsylinkInteractable") && !activePsylinks.Any(item => item.obj == hit.transform.gameObject) && !hit.transform.GetComponent<PsylinkInteractableObject>().isMoving;
